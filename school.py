...
class SchoolStandard(models.Model):
    ...
    def check_standard_unique(self):
        standard_search = self.env['school.standard'
                                             ...
                                             ('id', 'not in', self.ids)])
        if standard_search:
            raise ValidationError(_('''Kelas tidak boleh sama!'''
                                    ))
    ...
    def unlink(self):
        for rec in self:
            if rec.student_ids or rec.subject_ids or rec.syllabus_ids:
                raise ValidationError(_('''Tidak bisa melakukan operasi delete!'''))
        return super(SchoolStandard, self).unlink()

    @api.constrains('capacity')
    def check_seats(self):
        if self.capacity <= 0:
            raise ValidationError(_('''Jumlah kuota kelas harus lebih dari 0!'''))
